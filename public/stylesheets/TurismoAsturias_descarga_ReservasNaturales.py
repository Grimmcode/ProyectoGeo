...
#Enlace de rutas 
url_reservas_naturales = 'https://www.turismoasturias.es/naturaleza/reservas-naturales'

#Obtenemos el contenido de la url
contenido_url_naturales = requests.get(url_reservas_naturales)

#Hacemos un parseo para obtener el contenido en formato html
html_naturales = BeautifulSoup(contenido_url_naturales.content, 'html.parser')
...
etiquetas_rutas = html_naturales.find_all('a', class_='link')
...
while contador <= len(nombres):
    ...
    for i in range(len(enlaces_rutas)):
        ...
        with open('reservasNaturales.txt','a') as reservasNaturales:
