...
url_rutas_CS_prim = 'https://www.turismoasturias.es/camino-de-santiago/camino-primitivo'

#Obtenemos el contenido de la url
contenido_url_CS_prim = requests.get(url_rutas_CS_prim)

#Hacemos un parseo para obtener el contenido en formato html
html_CS_prim = BeautifulSoup(contenido_url_CS_prim.content, 'html.parser')
...
etiquetas_rutas = html_CS_prim.find_all('a', class_='link')
...
etiquetas_nombre = html_CS_prim.find_all('span', class_="title")
...
while contador < len(nombres_archivos):
    ...
    for i in range(len(enlaces_rutas)):
        ...
        paginas_rutas_CS_prim = requests.get(enlaces_rutas[i])
        ...
        pagina_unica = paginas_rutas_CS_prim
        #@--> comprueba que la conexion se realiza correctamente, resultado: response[200]
        ...
        for rutas in etiquetas_kml:
            ...
            ruta_carpeta ='D:/Proyectos_Trabajos/ProyAppGeo/Proyecto_2021_2022/datos/TurismoAsturias/caminoSantiago/caminoPrimitivo/'+nombres[contador] +".kml"
            contador = contador +1
