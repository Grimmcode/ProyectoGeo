...
#Enlace de rutas 
url_playas = 'https://www.turismoasturias.es/costa/playas'

#Obtenemos el contenido de la url
contenido_url_playas = requests.get(url_playas)

#Hacemos un parseo para obtener el contenido en formato html
html_playas = BeautifulSoup(contenido_url_playas.content, 'html.parser')
...
etiquetas_rutas = html_playas.find_all('a', class_='link')
...
while contador <= len(nombres):
    ...
    for i in range(len(enlaces_rutas)):
        ...
        paginas_rutas_playas = requests.get(enlaces_rutas[i])
        ...
        pagina_unica = paginas_rutas_playas
        #@--> comprueba que la conexion se realiza correctamente, resultado: response[200]
        ...
        with open('playas.txt','a') as reservasNaturales:
