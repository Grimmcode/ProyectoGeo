...
#Enlace de rutas 
url_lugares_interes = 'https://www.turismoasturias.es/descubre/cultura/patrimonio-industrial/recursos-y-equipamientos'

#Obtenemos el contenido de la url
contenido_url_lugares_interes = requests.get(url_lugares_interes)

#Hacemos un parseo para obtener el contenido en formato html
html_lugares_interes = BeautifulSoup(contenido_url_lugares_interes.content, 'html.parser')
...
etiquetas_rutas = html_lugares_interes.find_all('a', itemprop='url')
...
etiquetas_nombre = html_lugares_interes.find_all('span', itemprop="name")
...
while contador <= len(nombres):
    ...
    for i in range(len(enlaces_rutas)):
        ...
        paginas_rutas_lugares_interes = requests.get(enlaces_rutas[i])
        #@--> comprueba que la conexion se realiza correctamente, resultado: response[200]
        #@print("paginas todas: ", paginas_rutas_lugares_interes)
        ...
        pagina_unica = paginas_rutas_lugares_interes
        #@--> comprueba que la conexion se realiza correctamente, resultado: response[200]
        ...
        with open('lugaresInteres.txt','a') as reservasBiosfera:
