...
#Enlace de rutas 
url_patrimonio_humanidad = 'https://www.turismoasturias.es/cultura/patrimonio-humanidad'

#Obtenemos el contenido de la url
contenido_url_patrimonio = requests.get(url_patrimonio_humanidad)

#Hacemos un parseo para obtener el contenido en formato html
html_patrimonio = BeautifulSoup(contenido_url_patrimonio.content, 'html.parser')
...
etiquetas_rutas = html_patrimonio.find_all('a', class_='link')

#obtemos las url de cada una de las rutas 
for enlaces in etiquetas_rutas:
    enlaces_rutas.append(enlaces.get("href"))
##eliminamos el enlace al camino de santiago, ya que este no contiene coordenadas especificas al ser una ruta
enlaces_rutas.remove("https://www.turismoasturias.es/descubre/cultura/itinerarios-culturales/camino-de-santiago")



# ---> Obtencion de nombres para los archivos
#Obtenemos las etiquetas con los nombres de los archivos
etiquetas_nombre = html_patrimonio.find_all('span', class_="title")
...
for j in range(len(nombres_limpios)):
    ...
    ##eliminamos el nombre del camino de satiago
nombres.remove("ElCaminodeSantiago")
...
while contador < len(nombres):
    ...
    for i in range(len(enlaces_rutas)):
        ...
        paginas_rutas_naturales = requests.get(enlaces_rutas[i])
        ...
        pagina_unica = paginas_rutas_naturales
        #@--> comprueba que la conexion se realiza correctamente, resultado: response[200]
        ...
        with open('patrimonioHumanidad.txt','a') as reservasNaturales:
            reservasNaturales.write(nombres[contador] + "\nCoord: " + coordenadas + "\n\n")
            contador = contador + 1